...
import { enlacesGetId, selectEnlacesById,  updateEnlace} from '../../store/app/enlaceSlice';
...
function EnlacesTypeformFocus(props) {
	const dispatch = useDispatch();
	const classes = useStyles();
	const routeParams = useParams();
  const enlace = useSelector( state => selectEnlacesById(state, routeParams.enlaceId));
  ...
  const initText = useCallback(() => {
	...
	useEffect(() => {
		dispatch(enlacesGetId(routeParams.enlaceId));
    if(enlace){
      initText();
    }
	}, [dispatch, routeParams, enlace]);

  const handleSubmitBlog = () => {
    dispatch(updateEnlace(form));
	};
	return (
    <FuseAnimate delay={100} >
      <div >
        <Typography className={clsx('mb-16 py-12 px-36', classes.headerCard)} color="textPrimary" variant="h6">{enlace && enlace.texto}</Typography>
...
export default EnlacesTypeformFocus;